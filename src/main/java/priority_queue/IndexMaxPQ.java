...
public class IndexMaxPQ<Key extends Comparable<Key>> implements Iterable<Integer> {
    ...
    public void insert (int i, Key key) {
        validateIndex(i);
        if (contain(i)) throw new IllegalArgumentException("index is already in priority queue");
        n++;
        qp[i] = n;
        pq[n] = i;
        keys[i] = key;
    ...
    public Key keyOf (int i) {
        validateIndex(i);
        if (!contain(i)) throw new NoSuchElementException("index is not in the priority queue");
        else return keys[i];
    }
     ...
     * @throws IllegalArgumentException unless {@code 0 <= i < maxN}
     */
    public void changeKeys (int i, Key key) {
        validateIndex(i);
        if (!contain(i)) throw new NoSuchElementException("index is not in the priority queue");
        keys[i] = key;
        swim(qp[i]);
    ...
    public static void main(String[] args) {
        ...
        Integer[] perm = new Integer[strings.length];
        for (int i = 0; i < strings.length; i++)
            perm[i] = i;
